...
static struct in_addr server_id(struct dhcp_context *context,
		struct in_addr override, struct in_addr fallback);
...
static void do_options(struct dhcp_context *context, struct dhcp_packet *mess,
		unsigned char *end, unsigned char *req_options, char *hostname,
		char *domain, struct dhcp_netid *netid, struct in_addr subnet_addr,
		unsigned char fqdn_flags, int null_term, int pxe_arch,
		unsigned char *uuid, int vendor_class_len, time_t now,
...
size_t dhcp_reply(struct dhcp_context *context, char *iface_name, int int_index,
		size_t sz, time_t now, int unicast_dest, int loopback, int *is_inform,
	...
	if ((opt = option_find(mess, sz, OPTION_MESSAGE_TYPE, 1))) {
		...
		if ((opt = option_find(mess, sz, OPTION_AGENT_ID, 1))) {
			...
			unsigned char *last_opt = option_find1(
					&mess->options[0] + sizeof(u32),
			...
			if ((sopt = option_find1(option_ptr(opt, 0),
					option_ptr(opt, option_len(opt)), SUBOPT_SUBNET_SELECT,
					INADDRSZ)))
		...
		if ((opt = option_find(mess, sz, OPTION_CLIENT_ID, 1))) {
			...
			my_syslog(MS_DHCP | LOG_INFO,
					_("found client ID %2X:%2X:%2X:%2X:%2X:%2X:%2X"), clid[0],
					clid[1], clid[2], clid[3], clid[4], clid[5], clid[6]);
		} else if (clid && clid_len > 0) {
			my_syslog(MS_DHCP | LOG_INFO,
					_("no client ID but use %2X:%2X:%2X:%2X:%2X:%2X"), clid[0],
					clid[1], clid[2], clid[3], clid[4], clid[5]);
		} else {
			my_syslog(MS_DHCP | LOG_INFO,
					_(
							"client ID is empty hwaddr=%02X:%02X:%02X:%02X:%02X:%02X:"),
					mess->chaddr[0], mess->chaddr[1], mess->chaddr[2],
					mess->chaddr[3], mess->chaddr[4], mess->chaddr[5]);
		}

		/* do we have a lease in store? */
		lease = lease_find_by_client(mess->chaddr, mess->hlen, mess->htype,
				clid, clid_len);
		if (lease) {
			my_syslog(MS_DHCP | LOG_INFO, _("found lease"));
		} else {
			my_syslog(MS_DHCP | LOG_INFO, _("no lease found"));
		}
		...
		emac = extended_hwaddr(mess->htype, mess->hlen, mess->chaddr, clid_len,
				clid, &emac_len);
		/*
		 print_mac(daemon->namebuff, emac, emac_len);
		 my_syslog(MS_DHCP | LOG_INFO, _("MAC:%s", daemon->namebuff));
		 */
	}
	...
	for (mac = daemon->dhcp_macs; mac; mac = mac->next)
		if (mac->hwaddr_len == mess->hlen
				...
				&& memcmp_masked(mac->hwaddr, mess->chaddr, mess->hlen,
						mac->mask)) {
		...
		if (!context_new)
			for (context_tmp = daemon->dhcp; context_tmp; context_tmp =
				...
				if (!(context_tmp->flags & CONTEXT_NETMASK)
					...
					if (IN_CLASSA(
							ntohl(
									context_tmp->start.s_addr)) && IN_CLASSA(ntohl(context_tmp->end.s_addr)))
						netmask.s_addr = htonl(0xff000000);
					else if (IN_CLASSB(
							ntohl(
									context_tmp->start.s_addr)) && IN_CLASSB(ntohl(context_tmp->end.s_addr)))
						netmask.s_addr = htonl(0xffff0000);
					else if (IN_CLASSC(
							ntohl(
									context_tmp->start.s_addr)) && IN_CLASSC(ntohl(context_tmp->end.s_addr)))
	...
	if (option_bool(OPT_LOG_OPTS)) {
		...
		for (context_tmp = context; context_tmp;
			...
			if (context_tmp->flags & (CONTEXT_STATIC | CONTEXT_PROXY))
				my_syslog(MS_DHCP | LOG_INFO,
						_("%u available DHCP subnet: %s/%s"), ntohl(mess->xid),
						daemon->namebuff, inet_ntoa(context_tmp->netmask));
			else
				my_syslog(MS_DHCP | LOG_INFO,
	...
	for (o = daemon->dhcp_match; o; o = o->next) {
		...
		if (o->flags & DHOPT_RFC3925) {
			...
			for (offset = 0; offset < (option_len(opt) - 5u);
					offset += len + 5) {
				len = option_uint(opt, offset + 4, 1);
				/* Need to take care that bad data can't run us off the end of the packet */
				if ((offset + len + 5 <= (unsigned) (option_len(opt)))
						&& (option_uint(opt, offset, 4)
								== (unsigned int) o->u.encap))
					for (o2 = offset + 5; o2 < offset + len + 5;
							o2 += elen + 1) {
						elen = option_uint(opt, o2, 1);
						if ((o2 + elen + 1 <= option_len(opt)) && (match =
								match_bytes(o, option_ptr(opt, o2 + 1), elen)))
							break;
					}
				if (match)
					break;
	...
	for (vendor = daemon->dhcp_vendors; vendor; vendor = vendor->next) {
		...
		if ((opt = option_find(mess, sz, mopt, 1))) {
			...
			for (i = 0; i <= (option_len(opt) - vendor->len); i++)
				if (memcmp(vendor->data, option_ptr(opt, i), vendor->len)
						== 0) {
					vendor->netid.next = netid;
					netid = &vendor->netid;
					break;
		...
		if (!message) {
			...
			if (have_config(config, CONFIG_ADDR)) {
				...
				if ((lease = lease_find_by_addr(config->addr))
						&& (lease->hwaddr_len != mess->hlen
								...
								|| memcmp(lease->hwaddr, mess->chaddr,
										lease->hwaddr_len) != 0))
					message = _("address in use");
			} else {
				if (!(lease = lease_find_by_client(mess->chaddr, mess->hlen,
						mess->htype, NULL, 0))
					...
					if (address_allocate(context, &mess->yiaddr, mess->chaddr,
							mess->hlen, tagif_netid, now, loopback, iface_name)
							<= 0)
			...
			if (!message && !nailed) {
				for (id_list = daemon->bootp_dynamic; id_list;
					...
					if ((!id_list->list)
							|| match_netid(id_list->list, tagif_netid, 0))
						break;
				if (!id_list)
			...
			if (!message) {
				...
				lease_set_hwaddr(lease, mess->chaddr, NULL, mess->hlen,
						mess->htype, 0, now, 1);
				if (hostname)
		...
		if (strlen(client_hostname) != 0) {
			...
			if (!config) {
				...
				struct dhcp_config *new = find_config(daemon->dhcp_conf,
						context, NULL, 0, mess->chaddr, mess->hlen, mess->htype,
						hostname);
	...
	if (daemon->enable_pxe && (opt = option_find(mess, sz, OPTION_VENDOR_ID, 9))
		...
		if ((mess_type == DHCPREQUEST || mess_type == DHCPINFORM) && (opt =
				option_find(mess, sz, OPTION_VENDOR_CLASS_OPT, 1))
				&& (opt = option_find1(option_ptr(opt, 0),
						option_ptr(opt, option_len(opt)), SUBOPT_PXE_BOOT_ITEM,
						4))) {
			...
			if (strchr(service->basename, '.'))
				snprintf((char *) mess->file, sizeof(mess->file), "%s",
						service->basename);
			else
				...
				if (tmp) {
					...
					if (!workaround && boot) {
						...
						if (boot->next_server.s_addr)
							...
							mess->siaddr = a_record_from_hosts(boot->tftp_sname,
									now);
					...
					if ((pxe && !workaround) || !redirect4011)
						do_encap_opts(
								pxe_opts(pxearch, tagif_netid, tmp->local, now),
								OPTION_VENDOR_CLASS_OPT, DHOPT_VENDOR_MATCH,
								mess, end, 0);

					log_packet("PXE", NULL, emac, emac_len, iface_name,
							ignore ? "proxy-ignored" : "proxy", NULL,
							mess->xid);
					log_tags(tagif_netid, ntohl(mess->xid));
					if (!ignore)
						apply_delay(mess->xid, recvtime, tagif_netid);
					return ignore ?
							0 : dhcp_packet_size(mess, agent_id, real_end);
				}
			}
		}
	}
	...
	case DHCPDECLINE:
		if (!(opt = option_find(mess, sz, OPTION_SERVER_IDENTIFIER, INADDRSZ))
				|| option_addr(opt).s_addr
						!= server_id(context, override, fallback).s_addr)
			return 0;
	...
	case DHCPRELEASE:
		if (!(context = narrow_context(context, mess->ciaddr, tagif_netid))
				|| !(opt = option_find(mess, sz, OPTION_SERVER_IDENTIFIER,
						INADDRSZ))
				|| option_addr(opt).s_addr
						!= server_id(context, override, fallback).s_addr)
			return 0;
	...
	case DHCPDISCOVER:
		my_syslog(MS_DHCP | LOG_INFO, _("Processing DHCPDISCOVER"));
		...
		if (ignore || have_config(config, CONFIG_DISABLE)) {
			...
			if (have_config(config, CONFIG_ADDR)) {
				...
				if ((ltmp = lease_find_by_addr(config->addr)) && ltmp != lease
						&& !config_has_mac(config, ltmp->hwaddr,
								ltmp->hwaddr_len, ltmp->hwaddr_type)) {
					int len;
					unsigned char *mac = extended_hwaddr(ltmp->hwaddr_type,
							ltmp->hwaddr_len, ltmp->hwaddr, ltmp->clid_len,
							ltmp->clid, &len);
					my_syslog(MS_DHCP | LOG_WARNING,
							_(
...

				{
					...
					if (tmp)
						...
						my_syslog(MS_DHCP | LOG_WARNING,
								...
								addrs);
					else if (!do_icmp_ping(now, config->addr, 0, loopback, emac,
							emac_len, iface_name, &is_overload))
						my_syslog(MS_DHCP | LOG_WARNING,
								_(
										"not using configured address %s because it is in use by another host"),
								addrs);
					else if (is_overload)
						my_syslog(MS_DHCP | LOG_WARNING,
								_("ping overload, could not confirm the IP"));
					else
						conf = config->addr;
				}
			}
			...
			if (conf.s_addr) {
				char *addrs = inet_ntoa(conf);
				mess->yiaddr = conf;
				my_syslog(MS_DHCP | LOG_INFO,
						_("configured IP exist, offer IP %s"), addrs);
			} else if (lease
					&& address_available(context, lease->addr, tagif_netid)
					&& !config_find_by_address(daemon->dhcp_conf,
							lease->addr)) {
				char *addrs = inet_ntoa(lease->addr);
				mess->yiaddr = lease->addr;
				my_syslog(MS_DHCP | LOG_INFO, _("lease exist, offer IP %s"),
						addrs);
			} else if (opt && address_available(context, addr, tagif_netid)
					&& !lease_find_by_addr(addr)
					&& !config_find_by_address(daemon->dhcp_conf, addr)
					&& do_icmp_ping(now, addr, 0, loopback, emac, emac_len,
							iface_name, &is_overload)) {
				char *addrs = inet_ntoa(addr);
				my_syslog(MS_DHCP | LOG_INFO, _("lease not exist, offer IP %s"),
						addrs);
				if (is_overload)
					message = _("ping overload, could not confirm address");
				else
					mess->yiaddr = addr;
			} else if (emac_len == 0)
				message = _("no unique-id");
			else if (address_allocate(context, &mess->yiaddr, emac, emac_len,
					tagif_netid, now, loopback, iface_name) <= 0)
				message = _("no address available");
		}
	...
	case DHCPREQUEST:
		my_syslog(MS_DHCP | LOG_INFO, _("Processing DHCPREQUEST"));
		if (ignore || have_config(config, CONFIG_DISABLE))
			return 0;
		if ((opt = option_find(mess, sz, OPTION_REQUESTED_IP, INADDRSZ))) {
			my_syslog(MS_DHCP | LOG_INFO, _("Request have IP option"));
			...
			if ((opt = option_find(mess, sz, OPTION_SERVER_IDENTIFIER, INADDRSZ))) {
				my_syslog(MS_DHCP | LOG_INFO,
						_("Request is selecting (for OFFER message)"));
				/* SELECTING */
				selecting = 1;

				if (override.s_addr != 0) {
					char *addrs = inet_ntoa(override);
					my_syslog(MS_DHCP | LOG_INFO,
							_("override exist check IP %s"), addrs);
					if (option_addr(opt).s_addr != override.s_addr)
						return 0;
				} else {
					for (; context; context = context->current)
						if (context->local.s_addr == option_addr(opt).s_addr) {
							char *addrs = inet_ntoa(context->local);
							my_syslog(MS_DHCP | LOG_INFO,
									_("match local address (%s)"), addrs);
							break;
						}
				...
				if (lease && lease->addr.s_addr != mess->yiaddr.s_addr) {
					char *addrs = inet_ntoa(lease->addr);
					char *addrs1 = inet_ntoa(mess->yiaddr);
					my_syslog(MS_DHCP | LOG_WARNING,
							_(
									"lease address (%s) does not match requested address(%s)"),
							addrs1);
					lease_prune(lease, now);
					lease = NULL;
					...
					if (!lease) {
						my_syslog(MS_DHCP | LOG_INFO,
								_("lease does not exist in the request"));

						if (!do_icmp_ping(now, mess->yiaddr, 0, loopback, emac,
								emac_len, iface_name, &is_overload)) {
							my_syslog(MS_DHCP | LOG_WARNING,
									_("Required IP is used by other host"));
							message = _("address in use");
						} else {
							if (is_overload) {
								my_syslog(MS_DHCP | LOG_WARNING,
										_("ping overload, ignore the request"));
								return 0;
							}
						}
					} else {
						my_syslog(MS_DHCP | LOG_INFO,
								_("lease information matched the required IP"));
					}
				}
			} else {
				/* INIT-REBOOT */
				my_syslog(MS_DHCP | LOG_INFO, _("Request is INIT-REBOOT"));
				if (!lease && !option_bool(OPT_AUTHORITATIVE)) {
					my_syslog(MS_DHCP | LOG_WARNING,
							_("there is no lease for INIT-REBOOT"));
					return 0;
				}

				if (lease && lease->addr.s_addr != mess->yiaddr.s_addr) {
					my_syslog(MS_DHCP | LOG_WARNING,
							_("there is a lease but mismatch the IP"));
					message = _("wrong address");
				} else {
					my_syslog(MS_DHCP | LOG_INFO,
							_("there is a lease and match the IP "));
					if (!do_icmp_ping(now, mess->yiaddr, 0, loopback, emac,
							emac_len, iface_name, &is_overload)) {
						my_syslog(MS_DHCP | LOG_WARNING,
								_("This IP is used by other host"));
						message = _("address in use");
					} else {
						if (is_overload) {
							my_syslog(MS_DHCP | LOG_WARNING,
			...
			my_syslog(MS_DHCP | LOG_INFO, _("Request RENEWING or REBINDING"));
		...
		if (!message) {
			...
			/* Check if a new static address has been configured. Be very sure that
			 ...
			 an endless protocol loop will ensue. */
					...
					&& config->addr.s_addr != mess->yiaddr.s_addr && (!(ltmp =
							lease_find_by_addr(config->addr)) || ltmp == lease))
				message = _("static lease available");

			/* Check to see if the address is reserved as a static address for another host */
			else if ((addr_config = config_find_by_address(daemon->dhcp_conf,
					mess->yiaddr)) && addr_config != config)
		...
		if (message) {
			log_packet("DHCPNAK", &mess->yiaddr, emac, emac_len, iface_name,
					NULL, message, mess->xid);
			...
			if (!hostname_auth) {
				for (id_list = daemon->dhcp_ignore_names; id_list; id_list =
						id_list->next)
					if ((!id_list->list)
							|| match_netid(id_list->list, tagif_netid, 0))
						break;
			...
			if (!hostname && emac_len != 0) {
				for (id_list = daemon->dhcp_gen_names; id_list; id_list =
						id_list->next)
					if ((!id_list->list)
							|| match_netid(id_list->list, tagif_netid, 0))
						break;
				if (id_list) {
					...
					for (i = 0; (i < emac_len) && (i < 80); i++)
						hostname += sprintf(hostname, "%.2x%s", emac[i],
								(i == emac_len - 1) ? "" : "-");
					hostname = daemon->dhcp_buff;
				}
			}
	...
	case DHCPINFORM:
		...
		if (!lease && (lease = lease_find_by_addr(mess->ciaddr))
				&& lease->hostname)
...
static unsigned int calc_time(struct dhcp_context *context,
	...
	unsigned int time =
			have_config(config, CONFIG_TIME) ?
					config->lease_time : context->lease_time;
...
static void log_packet(char *type, void *addr, unsigned char *ext_mac,
	...
	if (option_bool(OPT_LOG_OPTS))
		my_syslog(MS_DHCP | LOG_INFO, "%u %s(%s) %s%s%s %s%s", ntohl(xid), type,
				interface, addr ? inet_ntoa(a) : "", addr ? " " : "",
				daemon->namebuff, string ? string : "", err ? err : "");
	else
		my_syslog(MS_DHCP | LOG_INFO, "%s(%s) %s%s%s %s%s", type, interface,
				addr ? inet_ntoa(a) : "", addr ? " " : "",
				daemon->namebuff, string ? string : "", err ? err : "");
}

static void log_options(unsigned char *start, u32 xid) {
	while (*start != OPTION_END) {
		char *optname = option_string(AF_INET, start[0], option_ptr(start, 0),
...
static unsigned char *option_find(struct dhcp_packet *mess, size_t size,
	...
	if ((ret = option_find1(&mess->options[0] + sizeof(u32),
			((unsigned char *) mess) + size, opt_type, minsize)))
		return ret;

	/* look for overload option. */
	if (!(overload = option_find1(&mess->options[0] + sizeof(u32),
			((unsigned char *) mess) + size, OPTION_OVERLOAD, 1)))
		return NULL;

	/* Can we look in filename area ? */
	if ((overload[2] & 1)
			&& (ret = option_find1(&mess->file[0], &mess->file[128], opt_type,
					minsize)))
		return ret;

	/* finally try sname area */
	if ((overload[2] & 2)
			&& (ret = option_find1(&mess->sname[0], &mess->sname[64], opt_type,
					minsize)))
		return ret;
	...
	for (; dopt; dopt = dopt->next) {
		...
		if (opt && (dopt->flags & DHOPT_VENDOR)) {
			...
			for (i = 0; i <= (option_len(opt) - len); i++)
				if (len == 0
						|| memcmp(dopt->u.vendor_class, option_ptr(opt, i), len)
								== 0) {
					dopt->flags |= DHOPT_VENDOR_MATCH;
					break;
				}
...
static int pxe_uefi_workaround(int pxe_arch, struct dhcp_netid *netid,
	...
	for (found = NULL, service = daemon->pxe_services; service; service =
			service->next)
	...
	if (found->sname) {
		...
		inet_ntop(AF_INET, &mess->siaddr, (char *) mess->sname,
				INET_ADDRSTRLEN);
	}
...
#define NUM_OPTS 4  
	...
	for (i = 0, service = daemon->pxe_services; service; service =
			service->next)
		if (pxe_arch == service->CSA && match_netid(service->netid, netid, 1)) {
			...
			if (p - (unsigned char *) daemon->dhcp_buff + len + 3 < 253) {
				...
				*(p++) = len;
				...
				toobig: my_syslog(MS_DHCP | LOG_ERR, _("PXE menu too large"));
			...
			if (boot_server.s_addr != 0) {
				if (q - (unsigned char *) daemon->dhcp_buff3 + 3 + INADDRSZ
						>= 253)
					goto toobig;
...
static void do_options(struct dhcp_context *context, struct dhcp_packet *mess,
	...
	tagif = option_filter(netid,
			context && context->netid.net ? &context->netid : NULL,
			config_opts);
	...
	if (option_bool(OPT_LOG_OPTS) && req_options) {
		...
		for (i = 0; req_options[i] != OPTION_END; i++) {
			...
			q += snprintf(q, MAXDNAME - (q - daemon->namebuff), "%d%s%s%s",
					req_options[i], strlen(s) != 0 ? ":" : "", s,
					req_options[i + 1] == OPTION_END ? "" : ", ");
			if (req_options[i + 1] == OPTION_END
	...
	if ((boot = find_boot(tagif))) {
		if (boot->sname) {
			if (!option_bool(OPT_NO_OVERRIDE) && req_options
					&& in_list(req_options, OPTION_SNAME))
				option_put_string(mess, end, OPTION_SNAME, boot->sname, 1);
			else
				strncpy((char *) mess->sname, boot->sname,
						sizeof(mess->sname) - 1);
		}
	...
	/* Use the values of the relevant options if no dhcp-boot given and
	 ...
	 dhcp-optsfile. */
	{
		if ((!req_options || !in_list(req_options, OPTION_FILENAME)) && (opt =
				option_find2(OPTION_FILENAME)) && !(opt->flags & DHOPT_FORCE)) {
			strncpy((char *) mess->file, (char *) opt->val,
					sizeof(mess->file) - 1);
			done_file = 1;
		}

		if ((!req_options || !in_list(req_options, OPTION_SNAME)) && (opt =
				option_find2(OPTION_SNAME)) && !(opt->flags & DHOPT_FORCE)) {
			strncpy((char *) mess->sname, (char *) opt->val,
					sizeof(mess->sname) - 1);
			done_server = 1;
		}
	...
	for (opt = config_opts; opt; opt = opt->next) {
		...
		if (opt->len == 0
				&& (optno == OPTION_NETMASK || optno == OPTION_BROADCAST
						|| optno == OPTION_ROUTER || optno == OPTION_DNSSERVER
		...
		len = do_opt(opt, NULL, context,
				(optno == OPTION_SNAME || optno == OPTION_FILENAME) ?
						1 : null_term);
		...
		if ((flags = (opt->flags & (DHOPT_ENCAPSULATE | DHOPT_RFC3925)))) {
			...
			for (len = 0, o = config_opts; o; o = o->next) {
				int outer =
						flags & DHOPT_ENCAPSULATE ?
								o->u.encap : OPTION_VENDOR_IDENT_OPT;
			...
			if (found) {
				if (flags & DHOPT_ENCAPSULATE)
					do_encap_opts(config_opts, opt->u.encap, DHOPT_ENCAP_MATCH,
							mess, end, null_term);
				else if (len > 250)
					my_syslog(MS_DHCP | LOG_WARNING,
							_(
							...
							opt->u.encap);
				else if ((p = free_space(mess, end, OPTION_VENDOR_IDENT_OPT,
						len + 5))) {
	...
	if ((force_encap || in_list(req_options, OPTION_VENDOR_CLASS_OPT))
			&& do_encap_opts(config_opts, OPTION_VENDOR_CLASS_OPT,
					DHOPT_VENDOR_MATCH, mess, end, null_term) && pxe_arch == -1
...
static void apply_delay(u32 xid, time_t recvtime, struct dhcp_netid *netid) {
	...
	if (!delay_conf)
		/* No match, look for one without a netid */
		for (delay_conf = daemon->delay_conf; delay_conf; delay_conf =
				delay_conf->next)
