...
int icmp_ping_check_arp(struct in_addr addr, unsigned char *emac, int emac_len, char *interface_name)
{
    int gotreply = 1;
    int s;
    ...
    strcpy(arpreq.arp_dev, interface_name);
    ...
    if (s >= 0)
    {
      if(ioctl(s, SIOCGARP, &arpreq) >= 0) 
      {
        ...
        {
          char buf[32];
          print_mac(buf, &arpreq.arp_ha.sa_data[0], 6);
          my_syslog(MS_DHCP | LOG_INFO, _("ICMP reply source MAC: %s"), buf);
          if(memcmp(emac, &arpreq.arp_ha.sa_data[0], emac_len) == 0)
          {
            my_syslog(MS_DHCP | LOG_INFO, _("Ignore ICMP reply from itself"));
            gotreply = 0;
          }
          //gotreply=1;
        } 
        ...
        {
          my_syslog(MS_DHCP | LOG_INFO, _("No complete ARP entry for this IP"));
          gotreply = -1;
        }
      }
      else
      {
        my_syslog(MS_DHCP | LOG_WARNING, _("Could not get ARP information"));
        gotreply = -1;
      }
      close(s);
    }
    ...
    {
      my_syslog(MS_DHCP | LOG_WARNING, _("Could not create socket"));
      gotreply = -1;
    }
    return gotreply;
}
