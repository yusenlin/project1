...
void dhcp_packet(time_t now, int pxe_fd)
{
  ...
  if ((relay = relay_reply4((struct dhcp_packet *)daemon->dhcp_packet.iov_base, ifr.ifr_name)))
	...
	return;
      ...
      my_syslog(MS_DHCP | LOG_INFO, _("lease_prune"));
      lease_prune(NULL, now); /* lose any expired leases */
      iov.iov_len = dhcp_reply(parm.current, ifr.ifr_name, iface_index, (size_t)sz, 
			       now, unicast_dest, loopback, &is_inform, pxe_fd, iface_addr, recvtime);
      my_syslog(MS_DHCP | LOG_INFO, _("lease_update_dns"));
      lease_update_dns(0);
      my_syslog(MS_DHCP | LOG_INFO, _("lease_update_file"));
...
struct ping_result *do_icmp_ping(time_t now, struct in_addr addr, unsigned int hash, int loopback, unsigned char *emac, int emac_len, char *interface_name, int * is_overload)
{
  static struct ping_result dummy;
  struct ping_result *r, *victim = NULL;
  int count, max = PING_MAX_NUM;
  *is_overload = 0;
  my_syslog(MS_DHCP | LOG_INFO, _("do_icmp_ping enter for %04X"), addr.s_addr); 
  ...
  /* this is not valid anymore, we use PING_MAX_NUM instead */
  for (count = 0, r = daemon->ping_results; r; r = r->next)
	...
	if (r->addr.s_addr == addr.s_addr)
        {
          if(icmp_ping_check_arp(addr, emac, emac_len, interface_name) == 0)
          {
             my_syslog(MS_DHCP | LOG_INFO, _("do_icmp_ping found in cache")); 
	     return r;
          }
          return NULL;
        }
  ...
  if ((count >= max) || option_bool(OPT_NO_PING) || loopback)
    {
      ...
      if(count >= max)
      {
         my_syslog(MS_DHCP | LOG_WARNING, _("icmp overloaded")); 
	 *is_overload = 1;
      }
      return &dummy;
    }
  else if (icmp_ping(addr) && icmp_ping_check_arp(addr, emac, emac_len, interface_name) > 0)
  {
    my_syslog(MS_DHCP | LOG_WARNING, _("do_icmp_ping address in use")); 
      ...
      if (victim)
	{
      ...
      my_syslog(MS_DHCP | LOG_INFO, _("do_icmp_ping exit")); 
      return victim;
    }
}
...
int address_allocate(struct dhcp_context *context,
		     ...
		     struct dhcp_netid *netids, time_t now, int loopback, char *interface_name)   
{
  ...
  for (pass = 0; pass <= 1; pass++)
      ...
      else
	{
	  ...
	  do {
		...
		if ((r = do_icmp_ping(now, addr, j, loopback, hwaddr, hw_len, interface_name, &is_overload)))
 		  {
                    if(is_overload)
                    {
                       return -1;
